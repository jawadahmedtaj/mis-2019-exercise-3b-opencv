...
public class MainActivity extends AppCompatActivity implements CvCameraViewListener2 {
        ...
        public void onManagerConnected(int status) {
            switch (status) {
                case LoaderCallbackInterface.SUCCESS: {
                    ...
                    noseCascade.load(initAssetFile("haarcascade_mcs_nose.xml"));
                }
                break;
                default: {
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        ...
        if (faceCascade != null) {
            faceCascade.detectMultiScale(gray, faces, 1.2, 0, 0, faceClassifierSize, new Size());
        }
        for (Rect face: faces.toArray()) {
            MatOfRect noses = new MatOfRect();
            Mat faceArea = gray.submat(new Rect(face.x, face.y, face.width, face.height));
            noseCascade.detectMultiScale(faceArea, noses, 1.2, 0, 0, noseClassifierSize, new Size());
            for (Rect nose: noses.toArray()) {
                Imgproc.circle(col, new Point(nose.x + nose.width / 2, nose.y + nose.height / 2),nose.width * 2, new Scalar(255, 0, 0),10);
            }
        }
        return col;
    }
